...
**Search fan-out is client-side, one chain per request.** `apps/web/src/lib/matching.ts` + `apps/web/src/api/client.ts` fan `/api/retailer-search` out to one retailer per request and merge. This is deliberate: the Worker parses each chain's catalog within the invocation, and fanning all chains out in a single request blows the **Workers Free-plan per-request CPU budget** (Cloudflare error 1102). Per-chain failures become entries in an `errors[]` array rather than throwing, so one bad chain never sinks the whole search. Search is multi-pass (full query → brand-only retry for chains that AND every token → EAN re-query when a ≥60% match reveals a barcode). Escape hatch for products a chain's search index omits: paste a product-page URL → `POST /api/resolve-url` — for the edge-blocked chains (AB, Kritikos) it resolves from their D1 catalog index by the SKU in the URL; every other chain scrapes the page live.

**The daily scrape runs off-edge.** Retailer WAFs (Akamai, etc.) block Cloudflare Worker egress IPs with 403/503, so the edge cron in `wrangler.toml` is **disabled** and the scrape runs from a residential IP via a launchd job (`apps/worker/scripts/scrape-local.ts`, reusing the Worker's adapters, writing straight to remote D1). The same job also **rebuilds two D1 discovery indexes**: `kritikos_catalog` (Kritikos ships no server-side search, and scanning its ~29 MB catalog on the edge blows the free-plan CPU budget) and `ab_catalog` (AB's Akamai WAF 403s the edge and its API is blocked through any proxy, so live edge search would need a paid headless-render call — the off-edge job crawls AB's ~12k-product catalog via its empty-free-text SAP-Hybris browse instead). The edge then answers **both** chains' search from D1 with a cheap `LIKE` (`searchKritikosCatalog` / `searchAbCatalog` in `index.ts`), and resolves their pasted URLs from the same tables (`resolveFromCatalog`). This makes the local job the **single writer** to `price_history`, `kritikos_catalog`, and `ab_catalog` — do not re-enable the edge cron without accounting for write races. See `apps/worker/scripts/README.md`.

Because AB and Kritikos are served entirely from those indexes and every other chain (Sklavenitis included — its WAF stopped blocking the edge) answers the Worker directly, **interactive edge search/resolve needs no residential proxy at all**: the former Scrape.do render path has been fully removed. The only residential dependency left is the off-edge daily job.
